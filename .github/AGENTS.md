...
- **abi3 build uses Python 3.13** (built with 3.13, but the resulting wheel runs on Python 3.12+ via stable ABI — the nanobind 3.12+ API floor is the binding constraint, not the build Python). CMake flag: `-DENABLE_ABI3=ON`. setup.py env var: `ENABLE_ABI3=1`.
...
- cp311 builds pin `python-version: '3.11.9'`. abi3 builds use `python-version: '3.13'` (latest patch, forward-compatible via stable ABI down to Python 3.12).
...
- **OpenCV on macOS**: Homebrew ships either `opencv4` or `opencv5` depending on tap version. The workflow auto-detects `opencv5` first then falls back to `opencv4` under `${BREW_PREFIX}/opt/opencv/lib/cmake/`, exports as `OPENCV_CMAKE_DIR`, and passes `-DOpenCV_DIR=${OPENCV_CMAKE_DIR}`. Do not hardcode `opencv4` — recent bottles (5.0.0) ship under `opencv5`.

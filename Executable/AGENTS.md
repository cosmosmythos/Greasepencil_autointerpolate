...
- **Cross-platform flags:** Windows `.pyd` (MSVC VS2022); macOS universal `x86_64;arm64` (deployment target 10.15); Linux `.so` with `-static-libgcc -static-libstdc++` + `$ORIGIN` rpath + optional static OpenMP for cross-distro.
